...
#include "riscv_frame_manager.hpp"
...
class RiscvDesc : public MachineDesc {
  ...
  private:
    ...
    RiscvInstr *prepareSingleChain(BasicBlock *, FlowGraph *);
    ...
    int getRegForRead( Temp, int, LiveSet *);
    // acquires a register to write the value of a variable
    int getRegForWrite( Temp, int, int, LiveSet *);
